...
public class VRMouseLook : MonoBehaviour {
	...
	void Awake() {
		...
		float y = vrCameraTransform.rotation.eulerAngles.y;
		vrCameraTransform.rotation.eulerAngles.Set (0, 0, 0);//y=0;
	...
	void Update () {
		bool rolled = false;
		bool pitched = false;
		if (Input.GetKey(HorizontalAndVerticalKey) || HorizontalAndVerticalKey == KeyCode.None) {
			pitched = true;
			if (enableYaw) {
